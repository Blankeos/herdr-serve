...
// Version should match your Go VERSION file
const VERSION = require("./package.json").version;
const BINARY_NAME = "herdr-serve";

function getPlatformInfo() {
  const platform = process.platform;
  const arch = process.arch;

  // Map Node.js platform/arch to release target triples (cargo-dist / goreleaser)
  const platformMap = {
    darwin: {
  ...
  return {
    ...
    url: `https://github.com/Blankeos/herdr-serve/releases/download/v${VERSION}/${BINARY_NAME}-${target}${extension}`,
  };
}
...
function logInstallFailure(error) {
  ...
  console.error("\nYou can install herdr-serve directly using:");
  console.error(
    [
...
async function install({ exitOnComplete = false } = {}) {
  try {
    ...
    console.log(`✅ herdr-serve v${VERSION} installed successfully!`);
